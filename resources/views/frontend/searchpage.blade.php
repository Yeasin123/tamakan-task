                    ...
                    <div class="col-sm-6 col-lg-4 col-xl-3 col-6" style="display: block;">
                        <div class="product-item">
                            <div class="product-thumb">
                                <a href="{{route('productdetail',$item->slug)}} "><img src="{{asset('images/product/'.$item->image_one)}}" alt="product"></a>
                                <span class="batch sale">Sale</span>
                                <a class="wish-link addWishlist" href="#" data-id="{{$item->id}}">
                            ...
                            <div class="product-content">
                                <h6><a href="{{route('productdetail',$item->slug)}}" class="product-title">{{$item->product_name}}</a></h6>
                                <p class="quantity">{{$item->product_quantity}}</p>
                                <div class="d-flex justify-content-between align-items-center">
                                    @if($item->discount_price == NULL)
                                        <div class="price">{{$item->selling_price}} ৳</div>
                                     @else 
                                        <div class="price">{{$item->discount_price}} ৳<del>{{$item->selling_price}} ৳</del></div>
                                     @endif
